// Package cli assembles the catapi command tree from the catapi
...
// NewApp assembles the kit application from the catapi domain. The
...
//
// To add a command, declare it in catapi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := catapi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(catapi.Domain{}).Register(app)
