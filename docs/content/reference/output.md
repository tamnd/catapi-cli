...
describes what users get. Pick a format with `-o`, or let catapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
catapi <command> -o table   # aligned columns for reading
catapi <command> -o jsonl   # one JSON object per line, for piping
catapi <command> -o json    # a single JSON array
catapi <command> -o csv     # spreadsheet friendly
catapi <command> -o tsv     # tab-separated
catapi <command> -o url     # just the URL column
catapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
catapi <command> --fields id,title,url
```
...
```bash
catapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
catapi <command>            # a table, because this is a terminal
catapi <command> | wc -l    # JSONL, because this is a pipe
```
